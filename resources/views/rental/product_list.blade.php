            ...
            <div class="col-lg-12">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">{{ ucfirst($category_name) }}</li>
                    </ol>
                </nav> 
            </div>
            <div class="col-lg-12">
                <div class="panel panel-default">                    
                    <div class="panel-heading">
                        <a class="btn btn-success" href="{{ url('rent_prod/add') .'/'. $rc_id  }}">Add New</a> &nbsp;&nbsp;
                        @if(count($records))
                                ...
                                <tbody>
                                    @if(count($records))
                                        @foreach($records as $record) 
                                            <tr id="rpid_{{ $record->rp_id }}">
                                                <td><input type="checkbox" class="chk" name="chk[]" value="{{ $record->rp_id }}"/></td>
                                                <td>{{ ucfirst($record->name) }}</td>
                                                <td><span class="badge">{{ ($record->rental_type == 1) ? 'Daily':'Hourly' }}</span></td>
                                                <td>{{ $record->rental_rate }}</td>
                                                <td>{{ $record->stock }}</td>
                                                <td>
                                                    @if($record->status == 1)
                                                ...
                                                <td><a href="{{ url('rent_prod') ."/edit/". $rc_id ."/". $record->rp_id }}">[Edit]</a></td>                        
                                            </td>
                                        @endforeach
                                    @else 
...
$(document).ready(function(){
    ...
    $("#delete").click(function(){
        ...
        $.ajax({
            type: 'POST',
            url: "{{ url('rent_prod') . '/delete' }}",
            data: {'_token': $("input[name='_token']").val(),'rp_id':_selected},
            dataType: 'json',
            beforeSend: function() {
